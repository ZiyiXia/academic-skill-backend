...
def poll_job(base_url: str, job_id: str, interval_sec: int, timeout_sec: int) -> dict[str, Any]:
    ...
    with httpx.Client(timeout=None) as client:
        while True:
            try:
                response = client.get(f"{base_url.rstrip('/')}/v1/blog/jobs/{job_id}")
...
def save_outputs(result_payload: dict[str, Any], output_root: Path) -> dict[str, str]:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Submit a blog generation job and poll every 10 seconds.")
    parser.add_argument("--paper-id", required=True)
    parser.add_argument("--force", action="store_true", help="Ignore cached S3 blog result and regenerate.")
    ...
    parser.add_argument("--output-dir", default=str(ROOT / "outputs" / "blog_tests"))
    args = parser.parse_args()

    with httpx.Client(timeout=args.request_timeout) as client:
        response = client.post(
            f"{args.base_url.rstrip('/')}/v1/blog/jobs",
            json={"paper_id": args.paper_id, "force": args.force},
        )
        response.raise_for_status()
    ...
    with httpx.Client(timeout=args.request_timeout) as client:
        result_response = client.get(f"{args.base_url.rstrip('/')}/v1/blog/jobs/{created['job_id']}/result")
